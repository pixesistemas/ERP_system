...
function seedDemo() {
  ...
  for(const p of products) db.prepare(`INSERT INTO productos(empresa_id,codigo,codigo_barra,descripcion,precio,iva,unidad,activo,updated_at) VALUES (?,?,?,?,?,?,?,1,CURRENT_TIMESTAMP) ON CONFLICT(empresa_id,codigo) DO UPDATE SET codigo_barra=excluded.codigo_barra,descripcion=excluded.descripcion,precio=excluded.precio,iva=excluded.iva,unidad=excluded.unidad,activo=1`).run(empresa.id,...p);
