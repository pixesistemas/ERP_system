...
function saveSettings(empresaId, input) {
  ...
  db.prepare(`
    INSERT INTO empresa_configuraciones (empresa_id, stock_policy, stock_alerts_enabled, stock_alert_dashboard, logo_url, recibo_doble_copia, pos_agrupar_por_codigo, remito_requiere_nota_pedido, aplicar_reglas_en, auto_eliminar_notax_dias, updated_at)
  ...
  `).run(empresaId, next.stockPolicy, next.stockAlertsEnabled ? 1 : 0, next.stockAlertDashboard ? 1 : 0, next.logoUrl || null, next.reciboDobleCopia ? 1 : 0, next.posAgruparPorCodigo ? 1 : 0, next.remitoRequiereNotaPedido ? 1 : 0, JSON.stringify(next.aplicarReglasEn), next.autoEliminarNotaXDias);
