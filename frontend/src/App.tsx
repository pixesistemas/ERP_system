...
import { OrderReturnsPage } from "./components/pages/OrderReturnsPage";
...
import { VendorClientsPage } from "./components/pages/VendorClientsPage";
import { MobileVendorPage } from "./mobile/MobileVendorPage";
import { MobileOrdersAdminPage } from "./components/pages/MobileOrdersAdminPage";
...
export function App() {
  ...
  /*
   * Si la app se abre desde los subdominios de vendedor/repartidor,
   * arranca directo en su pantalla móvil.
   */
  const [page, setPage] = useState<string>(() => {
    const host = typeof window !== "undefined" ? window.location.hostname : "";
    if (host.startsWith("vendedor.")) return "mobile-vendor";
    if (host.startsWith("repartidor.")) return "mobile-driver";
    return "dashboard";
  });
   ...
   */
  useEffect(() => {
    if (page === "pos" || page === "mobile-vendor" || page === "mobile-driver") setMenuAbierto(false);
  ...
  return <div className={"app-shell" + (menuAbierto ? "" : " menu-hidden")}>
    <aside className="sidebar">
      ...
      <nav className="erp-nav" onClick={() => { if (typeof window !== "undefined" && window.innerWidth <= 650) setMenuAbierto(false); }}>
        ...
        {session.modulos?.PREVENTA_MOVIL&&canSee("mobile-vendor")&&<a className={page === "mobile-vendor" ? "active" : ""} onClick={()=>setPage("mobile-vendor")}><Phone size={18}/> App vendedor</a>}
        {session.modulos?.PREVENTA_MOVIL&&canSee("mobile-orders")&&<a className={page === "mobile-orders" ? "active" : ""} onClick={()=>setPage("mobile-orders")}><ClipboardList size={18}/> Bandeja de pedidos</a>}
        {session.modulos?.PREVENTA_MOVIL&&canSee("delivery-routes")&&<a className={page === "delivery-routes" ? "active" : ""} onClick={()=>setPage("delivery-routes")}><Truck size={18}/> Reparto</a>}
        {session.modulos?.PREVENTA_MOVIL&&canSee("mobile-driver")&&<a className={page === "mobile-driver" ? "active" : ""} onClick={()=>setPage("mobile-driver")}><Phone size={18}/> App repartidor</a>}
        {session.modulos?.PREVENTA_MOVIL&&canSee("vendor-reports")&&<a className={page === "vendor-reports" ? "active" : ""} onClick={()=>setPage("vendor-reports")}><BarChart3 size={18}/> Reportes de vendedores</a>}
        ...
        {canSee("order-returns")&&<a className={page === "order-returns" ? "active" : ""} onClick={()=>setPage("order-returns")}><Undo2 size={18}/> Anular pedidos</a>}
        ...
        {session.modulos?.PREVENTA_MOVIL&&canSee("vendor-clients")&&<a className={page === "vendor-clients" ? "active" : ""} onClick={()=>setPage("vendor-clients")}><Users size={18}/> Cartera de clientes</a>}
